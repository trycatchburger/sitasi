...
try {
    ...
    // Now perform the automatic association for user ID 10 (KTA010) using direct SQL
    ...
    foreach ($submissionIds as $id) {
        ...
        $assocSql = "UPDATE submissions SET user_id = ? WHERE id = ?";
        $stmt = $conn->prepare($assocSql);
        if ($stmt) {
            $userId = 10;
            $submissionId = $id;
            ...
            if ($success) {
                ...
                echo "    - Association failed: " . $stmt->error . "\n";
            }
            $stmt->close();
        } else {
