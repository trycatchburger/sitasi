...
class SubmissionRepository extends BaseRepository
{
    ...
    public function findAll(int $page = 1, int $perPage = 0, string $sort = null, string $order = 'asc'): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email";
    ...
    public function findPending(int $page = 1, int $perPage = 10, string $sort = null, string $order = 'asc'): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Pending'";
    ...
    public function findApproved(): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Diterima' ORDER BY s.created_at DESC";
            $result = $this->conn->query($sql);
            if ($result === false) {
                throw new DatabaseException("Database query failed: " . $this->conn->error);
            }
    ...
    public function findRecentApproved(int $limit = 6): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Diterima' ORDER BY s.created_at DESC LIMIT ?";
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
    ...
    public function findRecentApprovedJournals(int $limit = 6): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Diterima' AND s.submission_type = 'journal' ORDER BY s.created_at DESC LIMIT ?";
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
    ...
    public function findForRepositoryManagement(): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status IN ('Diterima', 'Pending') ORDER BY s.created_at DESC";
            $result = $this->conn->query($sql);
            if ($result === false) {
                throw new DatabaseException("Database query failed: " . $this->conn->error);
            }
    ...
    public function findJournalSubmissions(int $page = 1, int $perPage = 10, string $sort = null, string $order = 'asc'): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.submission_type = 'journal'";
    ...
    {
        try {
            $stmt = $this->conn->prepare("SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.id = ?");
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
    ...
    {
        try {
            $stmt = $this->conn->prepare("SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.id = ?");
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
            $stmt->bind_param("i", $id);
    ...
    public function searchRecentApproved(string $search, int $limit = 6): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Diterima' AND (s.judul_skripsi LIKE ? OR s.nama_mahasiswa LIKE ?) ORDER BY s.created_at DESC LIMIT ?";
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
    ...
    public function searchRecentApprovedJournals(string $search, int $limit = 6): array
    {
        try {
            ...
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.status = 'Diterima' AND s.submission_type = 'journal' AND (s.judul_skripsi LIKE ? OR s.nama_mahasiswa LIKE ?) ORDER BY s.created_at DESC LIMIT ?";
            $stmt = $this->conn->prepare($sql);
            if (!$stmt) {
                throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
            }
    ...
    {
        try {
            $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, a.username as admin_username, (s.created_at != s.updated_at AND s.updated_at > DATE_ADD(s.created_at, INTERVAL 1 SECOND)) as is_resubmission, s.submission_type, anggota.tipe_member FROM submissions s LEFT JOIN admins a ON s.admin_id = a.id LEFT JOIN users_login ul ON s.user_id = ul.id LEFT JOIN anggota ON ul.email = anggota.email WHERE s.user_id = ?";
