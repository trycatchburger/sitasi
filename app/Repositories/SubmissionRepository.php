...
class SubmissionRepository extends BaseRepository
{
    ...
    public function findUnassociatedSubmissionsByUserDetails(string $name, string $email, string $nim = null): array
    {
        try {
            if ($nim) {
                $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, s.degree_level, s.is_resubmission, s.submission_type, s.author, s.publication_date FROM submissions s WHERE user_id IS NULL AND nama_mahasiswa = ? AND email = ? AND nim = ?";
                $stmt = $this->conn->prepare($sql);
                if (!$stmt) {
                    throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
                }
                ...
                $sql = "SELECT s.id, s.admin_id, s.serial_number, s.nama_mahasiswa, s.author_2, s.author_3, s.author_4, s.author_5, s.nim, s.email, s.dosen1, s.dosen2, s.judul_skripsi, s.abstract, s.program_studi, s.tahun_publikasi, s.status, s.keterangan, s.notifikasi, s.created_at, s.updated_at, s.user_id, s.degree_level, s.is_resubmission, s.submission_type, s.author, s.publication_date FROM submissions s WHERE user_id IS NULL AND nama_mahasiswa = ? AND email = ?";
                $stmt = $this->conn->prepare($sql);
                if (!$stmt) {
                    throw new DatabaseException("Statement preparation failed: " . $this->conn->error);
                }
