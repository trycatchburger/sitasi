  ...
  <form method="GET" action="<?= url('submission/repository_skripsi') ?>"
    ...
    <div class="md:col-span-6 sm:col-span-2 col-span-1">
      ...
      <input type="text" 
             ...
             class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 transition duration-150">
    </div>
...
<div class="flex justify-between items-center mb-6">
  <h2 class="text-xl font-semibold text-gray-800">Daftar Skripsi</h2>

  <!-- Tombol Pilihan Tampilan -->
  <div class="flex gap-2">
    <button id="gridBtn" class="px-3 py-1.5 rounded-md bg-green-600 text-white text-sm font-medium hover:bg-green-700 active">🟦 Grid</button>
  ...
  <div id="submissionContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-300">
    ...
    <?php foreach ($submissions as $submission): ?>
      <?php
        ...
        $titleLink = '<a href="' . url('submission/' . $detailRoute . '/' . $submission['id']) . '" class="text-green-70 hover:text-green-900 font-medium hover:underline">' . htmlspecialchars($submission['judul_skripsi']) . '</a>';
      ?>
...
<?php
$title = 'Repository Skripsi | Portal Unggah Mandiri';
