...
<style>
  ...
  <div class="mb-10 flex justify-center">
    <form method="GET" action="<?= url() ?>" class="flex w-full max-w-2xl shadow-sm rounded-full overflow-hidden border border-gray-300 bg-white">
      ...
      <button 
        type="submit" 
        class="bg-[#02367b] text-white px-6 hover:bg-[#02367b] transition flex items-center justify-center"
      >
  ...
  <?php if (!empty($recentSubmissions)): ?>
        ...
        <?php foreach ($recentSubmissions as $submission): ?>
          <?php
            ...
            $citation = $formattedName . '. (' . htmlspecialchars($submission['tahun_publikasi']) . '). ' .
                        '<a href="' . url('submission/detail/' . $submission['id']) . '" class="text-blue-600 font-semibold hover:text-blue-800 hover:underline">' .
    ...
    <div class="mt-6 text-center">
      <a href="<?= url('submission/repository') ?>" class="inline-block px-5 py-2 border border-[#02367b] text-[#02367b] rounded-full hover:bg-[#02367b] hover:text-white transition">
        Lihat Semua
      </a>
    </div>
  <?php else: ?>
