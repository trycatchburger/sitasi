...
<div class="bg-white shadow-md border border-gray-100 rounded-2xl p-6 mt-6 mb-6"> <!-- tambahkan mb-6 -->
  <div class="flex items-center mb-4">
    <div class="w-9 h-9 flex items-center justify-center bg-yellow-10 text-yellow-700 rounded-full mr-3">
...
<div class="flex justify-between items-center mb-6">
  ...
  <div class="flex gap-2">
    <button id="gridBtn" class="px-3 py-1.5 rounded-md bg-yellow-600 text-white text-sm font-medium hover:bg-yellow-70 active">🟦 Grid</button>
  ...
  <div id="submissionContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 transition-all duration-300">
    ...
    <?php foreach ($submissions as $submission): ?>
      <?php
        ...
        $titleLink = '<a href="' . url('submission/' . $detailRoute . '/' . $submission['id']) . '" class="text-yellow-700 hover:text-yellow-900 font-medium hover:underline">' . htmlspecialchars($submission['judul_skripsi']) . '</a>';
      ?>
    ...
    <div class="flex items-center justify-center space-x-2">
      ...
      <?php
        ...
        for ($i = $startPage; $i <= $endPage; $i++) {
          if ($i == $currentPage) {
            echo '<span class="px-4 py-2 bg-yellow-600 text-white border border-yellow-600 rounded-lg">' . $i . '</span>';
          } else {
